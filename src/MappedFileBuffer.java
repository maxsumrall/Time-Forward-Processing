...
/**
     ...
     *                      with read-only access.
     *
     *	@param size 		The size of the file for situations where the file is empty
     *  @throws IllegalArgumentException if <code>segmentSize</code> is > 1GB.
     */
        public MappedFileBuffer(File file, int segmentSize, boolean readWrite, long size)
    throws IOException
    {
        if (segmentSize > MAX_SEGMENT_SIZE)
            throw new IllegalArgumentException(
...
//----------------------------------------------------------------------------
    ...
    public MappedFileBuffer clone()
    {
        ...
        {
            //throw new Exception("I used to implement Cloneable, why don't I now?");
